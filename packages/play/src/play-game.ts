 ...
 */
import { Game, GamePlayer, GameResult, GameState, TerminalUI } from '@ai-gamedev/playtest'
 ...
 */
export async function playGame(player: GamePlayer, options: PlayGameOptions = {}): Promise<GameResult | null> {
  ...
  try {
    ...
    try {
      ...
      ui.display(color.red(`Game terminated: ${error instanceof Error ? error.message : String(error)}`))
    ...
    if (result) {
      ...
      ui.display(`Result: ${result.success ? color.green('Victory!') : color.red('Defeat')}`)
